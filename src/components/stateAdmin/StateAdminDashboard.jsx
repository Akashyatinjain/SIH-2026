...
export default function StateAdminDashboard() {
  ...
  const handleDispatchStateResource = (dist) => {
    ...
    alert(`State Disaster Contingency: ₹25 Lakhs & 10,000 Ring Vaccine Doses released to ${dist.district} District Collectorate!`);
  };

  return (
    <div className="space-y-6 text-slate-900">
      {/* 1. Header: State Command Directorate Banner */}
      <div className="p-5 bg-gradient-to-r from-purple-950 via-slate-900 to-indigo-950 text-white rounded-2xl shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 px-2.5 py-1 bg-white/10 rounded-full text-xs text-purple-200 border border-white/10 mb-1.5">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
            <span>State Directorate of Animal Husbandry • Maharashtra Command</span>
          </div>
          <h2 className="text-xl sm:text-2xl font-black">Maharashtra Animal Health Intelligence</h2>
      ...
      {/* 2. STATE SIGNAL & DECISION SUPPORT QUESTIONS (Requirement 24) */}
      <div className="p-5 bg-midnight text-white rounded-2xl border border-slate-800 shadow-md space-y-4">
        <div className="flex items-center justify-between border-b border-slate-800 pb-2">
          <div className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-purple-400" />
            <h3 className="font-extrabold text-sm text-white">STATE SIGNAL & STRATEGIC DECISION SUPPORT</h3>
          </div>
          <span className="text-xs font-bold text-red-400 bg-red-950 px-2.5 py-0.5 rounded border border-red-800">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
          ...
          <div className="p-3.5 bg-slate-900 rounded-xl border border-slate-800 space-y-1">
            <span className="text-[10px] font-bold text-slate-400 uppercase">WHERE IS RISK INCREASING?</span>
            <p className="font-extrabold text-red-400 text-sm">Baramati & Daund</p>
            <p className="text-[11px] text-slate-400">Sugarcane vector fly surge (+28%)</p>
          </div>

          {/* Question 2 */}
          <div className="p-3.5 bg-slate-900 rounded-xl border border-slate-800 space-y-1">
            <span className="text-[10px] font-bold text-slate-400 uppercase">WHERE IS VACCINATION BELOW TARGET?</span>
            <p className="font-extrabold text-amber-400 text-sm">3 Districts Lagging</p>
          ...
          <div className="p-3.5 bg-slate-900 rounded-xl border border-slate-800 space-y-1">
            ...
            <p className="font-extrabold text-teal-400 text-sm">7 Priority Blocks</p>
          ...
          <div className="p-3.5 bg-slate-900 rounded-xl border border-slate-800 space-y-1">
            ...
            <p className="font-extrabold text-purple-300 text-sm">LSD, FMD, PPR</p>
            <p className="text-[11px] text-slate-400">Pre-monsoon ring vaccination active</p>
          </div>
        </div>
      </div>

      {/* 3. MAIN HERO: LARGE MAHARASHTRA RISK MAP & DISTRICT COMPARISON */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 bg-midnight text-white rounded-2xl border border-slate-800 p-5 shadow-md flex flex-col justify-between space-y-4">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-extrabold text-sm text-slate-100">Maharashtra District Risk Heatmap</h3>
            </div>
            <span className="text-[10px] font-mono text-slate-400">36 Districts</span>
          </div>

          {/* SVG Map of Maharashtra Districts */}
          <div className="h-64 flex items-center justify-center relative my-2">
            <svg viewBox="0 0 500 350" className="w-full h-full">
              <path d="M 80 120 L 180 40 L 320 60 L 440 100 L 480 190 L 400 290 L 260 320 L 140 290 L 60 210 Z" fill="#0f172a" stroke="#334155" strokeWidth="2" strokeDasharray="3 3" />

              {/* Pune (Critical 🔴) */}
              <polygon points="170,170 240,160 250,220 180,230" fill="#7f1d1d" stroke="#ef4444" strokeWidth="2" className="cursor-pointer hover:opacity-80" onClick={() => setSelectedDistrict(stateDistricts[0])} />
              <text x="195" y="195" fill="#ffffff" fontSize="11" fontWeight="bold">Pune</text>

              {/* Ahmednagar (Elevated 🟠) */}
              <polygon points="190,110 270,100 280,160 200,170" fill="#7c2d12" stroke="#ea580c" strokeWidth="1.5" className="cursor-pointer hover:opacity-80" onClick={() => setSelectedDistrict(stateDistricts[1])} />
              <text x="215" y="135" fill="#fed7aa" fontSize="10">Ahmednagar</text>

              {/* Solapur (Elevated 🟠) */}
              <polygon points="250,220 330,210 340,280 260,290" fill="#7c2d12" stroke="#ea580c" strokeWidth="1.5" className="cursor-pointer hover:opacity-80" onClick={() => setSelectedDistrict(stateDistricts[2])} />
              <text x="280" y="250" fill="#fed7aa" fontSize="10">Solapur</text>

              {/* Satara (Low 🟢) */}
              <polygon points="140,230 200,230 190,290 130,280" fill="#064e3b" stroke="#10b981" strokeWidth="1.5" className="cursor-pointer hover:opacity-80" onClick={() => setSelectedDistrict(stateDistricts[4])} />
              <text x="145" y="260" fill="#a7f3d0" fontSize="9">Satara</text>

              {/* Kolhapur (Low 🟢) */}
              <polygon points="130,280 190,290 180,330 120,320" fill="#064e3b" stroke="#10b981" strokeWidth="1.5" className="cursor-pointer hover:opacity-80" onClick={() => setSelectedDistrict(stateDistricts[6])} />
              <text x="135" y="310" fill="#a7f3d0" fontSize="9">Kolhapur</text>

              {/* Nashik (Watch 🟡) */}
              <polygon points="130,70 200,60 190,120 120,130" fill="#78350f" stroke="#f59e0b" strokeWidth="1.5" className="cursor-pointer hover:opacity-80" onClick={() => setSelectedDistrict(stateDistricts[5])} />
        ...
        <div className="lg:col-span-7 bg-white rounded-2xl border border-sand p-5 card-elevated space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-sand pb-3">
            <div>
              <h3 className="font-extrabold text-sm text-slate-900">DISTRICT COMPARISON & SURVEILLANCE RANKING</h3>
