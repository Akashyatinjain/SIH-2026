...
export default function GISDiseaseMap() {
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-bold text-sm text-slate-900">{t.gisMapTitle}</h3>
        ...
        <div className="flex flex-wrap items-center gap-1.5 text-xs">
          <button
            ...
            className={`px-2.5 py-1 rounded-lg border font-semibold transition flex items-center gap-1 ${
      ...
      {/* Main Map Visual Canvas & Hotspot Inspector Side-by-Side */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Interactive Pune District SVG Map (2 Cols on large screens) */}
        <div className="lg:col-span-2 bg-slate-950 rounded-2xl border border-slate-800 p-4 sm:p-6 text-white relative min-h-[460px] flex flex-col justify-between overflow-hidden shadow-md">
          {/* Map Title overlay */}
          ...
          <div className="relative w-full h-80 my-2 flex items-center justify-center">
            <svg viewBox="0 0 800 500" className="w-full h-full max-h-[380px]">
              ...
              <polygon points="380,310 560,330 580,430 390,420" fill="#3f1219" stroke="#ef4444" strokeWidth="2.5" className="hover:fill-red-950 cursor-pointer transition" />
              ...
              {/* Block 9: Indapur */}
              <polygon points="560,330 700,280 720,400 580,430" fill="#332a15" stroke="#f59e0b" strokeWidth="1.5" className="hover:fill-amber-950 cursor-pointer transition" />
              ...
              {/* Active Pulsing Danger Circles (HOTSPOTS) */}
              {activeLayers.hotspots && (
                <>
                  {/* Baramati Hotspot */}
                  <circle cx="480" cy="370" r="45" fill="url(#hotspotGradRed)" className="animate-pulse" />
                  <circle cx="480" cy="370" r="10" fill="#ef4444" stroke="#ffffff" strokeWidth="2" />
                  <text x="495" y="365" fill="#ffffff" fontSize="11" fontWeight="bold">7 Cases (LSD)</text>

                  {/* Daund Hotspot */}
                  <circle cx="460" cy="275" r="35" fill="url(#hotspotGradRed)" className="animate-pulse" />
                  <circle cx="460" cy="275" r="8" fill="#ef4444" stroke="#ffffff" strokeWidth="2" />
                  <text x="475" y="270" fill="#ffffff" fontSize="10" fontWeight="bold">5 Cases (Avian)</text>

                  {/* Shirur Hotspot */}
                  <circle cx="440" cy="180" r="28" fill="url(#hotspotGradAmber)" />
                  <circle cx="440" cy="180" r="7" fill="#f59e0b" stroke="#ffffff" strokeWidth="1.5" />
                  <text x="455" y="180" fill="#fef08a" fontSize="10">3 Cases (HS)</text>
              ...
              {activeLayers.hospitals && (
                <g>
                  ...
                  <text x="482" y="398" fill="#bfdbfe" fontSize="9" fontWeight="bold">🏥 Baramati Sub-Dist Hosp</text>
                </g>
              )}
            </svg>
          </div>
          ...
          {/* Map Legend Bar */}
          <div className="bg-slate-900/90 backdrop-blur-md p-3 rounded-xl border border-slate-800 flex flex-wrap items-center justify-between text-[11px] gap-2">
            ...
            <span className="text-slate-400 font-mono">Projection: WGS84 EPSG:4326</span>
          </div>
        </div>

        {/* Hotspot Inspector Panel (1 Col) */}
        <div className="bg-white rounded-2xl border border-slate-200 p-5 card-elevated flex flex-col justify-between space-y-4">
          <div>
              ...
              <div className="space-y-3 text-xs">
                ...
                <div className="p-3 bg-red-50 rounded-xl border border-red-200 space-y-1.5 text-red-950">
                  ...
                  <div className="flex justify-between">
                    ...
                    <strong>{selectedHotspot.villagesAffected.join(', ')}</strong>
                  </div>
                  <div className="flex justify-between">
          ...
          <div className="space-y-2 pt-2 border-t border-slate-100">
            ...
            >
              ...
              <span>{t.deployRRT}</span>
            </button>

            <button
              onClick={() => alert(`Broadcasting Ring SMS Advisory to all 1,420 livestock farmers in ${selectedHotspot.block} block!`)}
              className="w-full py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl font-semibold text-xs transition"
            >
